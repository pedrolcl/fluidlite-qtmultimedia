...
void ProgramSettings::internalRead(QSettings &settings)
{
    ...
    m_soundFontFile = settings.value("SoundFont", "default.sf2").toString();
    emit ValuesChanged();
}
...
void ProgramSettings::internalSave(QSettings &settings)
{
    ...
    settings.setValue("SoundFont", m_soundFontFile);
    settings.sync();
}

const QString &ProgramSettings::soundFontFile() const
{
    return m_soundFontFile;
}

void ProgramSettings::setSoundFontFile(const QString &newSoundFontFile)
{
    m_soundFontFile = newSoundFontFile;
}
